...
class TestServiceApi(TestCase):
    ...
    @patch('air_sdk.service.ServiceApi.create')
    def test_create_service(self, mock_create):
        res = self.api.create_service('abc123', 'test', 'intf', 22, foo='bar')
        mock_create.assert_called_with(
            simulation='abc123', name='test', interface='intf', dest_port=22, foo='bar'
        )
    ...
    def test_get(self, mock_raise):
        ...
        self.client.get.assert_called_with(f'{self.client.api_url}/service/abc123/', params={'foo': 'bar'})
    ...
    @patch('air_sdk.util.raise_if_invalid_response')
    @patch('air_sdk.service.ServiceApi._resolve_interface')
    def test_create(self, mock_resolve, mock_raise):
        sim = air_model.AirModel(MagicMock(), id='xyz123')
        ...
        self.client.post.assert_called_with(
            f'{self.client.api_url}/service/',
            json={'name': 'abc123', 'simulation': sim, 'interface': mock_resolve.return_value},
        )
    ...
    def test_create_id(self, mock_resolve, mock_raise):
        ...
        self.client.post.assert_called_with(
            f'{self.client.api_url}/service/',
            json={'name': 'abc123', 'simulation': 'xyz123', 'interface': 'test123'},
        )
