...
class TestAirModel(TestCase):
    ...
    @patch('air_sdk.air_model.AirModel.model_keys', {'lazy_list': 'lazy_api'})
    @patch('air_sdk.air_model.LazyLoadedList.__getitem__')
    ...
    @patch('air_sdk.air_model.AirModel.model_keys', {'lazy_lazy': 'lazy_api'})
    ...
    @patch('air_sdk.air_model.AirModel.model_keys', {'lazy_item': 'lazy_api'})
    ...
    def test_repr(self, mock_raise):
        self.assertRegex(str(self.model), r'<air_sdk.air_model.AirModel object at 0x[0-9a-f]+>')

    def test_repr_deleted(self, mock_raise):
        self.model._deleted = True
        self.assertRegex(
            str(self.model),
            r'<Deleted Object \(<air_sdk.air_model.AirModel ' + r'object at 0x[0-9a-f]+>\)>',
        )
    ...
    def test_getattribute_deleted(self, mock_raise):
        self.model._deleted = True
        with self.assertRaises(exceptions.AirObjectDeleted) as err:
            _ = self.model.foo
        self.assertEqual(
            err.exception.message,
            "<class 'air_sdk.air_model.AirModel'> object has "
    ...
    @patch('air_sdk.air_model.AirModel._patch')
    ...
    @patch('air_sdk.air_model.AirModel.model_keys', {'simulation': 'simulations'})
    def test_get_model_key(self, mock_raise):
        self.assertEqual(self.model._get_model_key('simulation'), 'simulations')

    @patch('air_sdk.air_model.AirModel.model_keys', {'simulation': {'Node': 'a', 'Node2': 'b'}})
    ...
    def test_delete(self, mock_raise):
        ...
        mock_raise.assert_called_with(
            self.model._api.client.delete.return_value, data_type=None, status_code=204
        )
    ...
    def test_json_lazy_loaded_list(self, mock_raise):
        self.model.test = air_model.LazyLoadedList(
            [air_model.LazyLoaded('a', 1), air_model.LazyLoaded('b', 2)], MagicMock()
        )
    ...
    def test_json_datetime(self, mock_raise):
        ...
        self.assertEqual(self.model.json(), '{"foo": "bar", "id": "abc123", "test": "2030-12-12T22:05:03"}')
