...
class TestAirModel(TestCase):
    ...
    def test_repr(self, mock_raise):
        self.assertRegex(str(self.model), r'<air_sdk.air_sdk.air_model.AirModel object at 0x[0-9a-f]+>')

    def test_repr_deleted(self, mock_raise):
        self.model._deleted = True
        self.assertRegex(
            str(self.model),
            r'<Deleted Object \(<air_sdk.air_sdk.air_model.AirModel ' + r'object at 0x[0-9a-f]+>\)>',
        )
    ...
    @patch('air_sdk.air_sdk.air_model.AirModel.model_keys', {'simulation': {'Node': 'a', 'Node2': 'b'}})
